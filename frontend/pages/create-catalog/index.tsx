...
//     catalog and its products in one call. Free tier is capped at
//     FREE_PRODUCT_LIMIT products per catalog server-side; the response's
//     `planLimit` field reports if this import got truncated.
//   Paid plan choice: PUT /users/upgrade-plan — records the choice
//     directly (no payment gateway exists yet), called before creation so
...
export default function CreateCatalogWizard() {
  ...
  // >FREE_PRODUCT_LIMIT products: ask which plan. At or under: every plan
  // covers that many anyway, so skip the ask and create straight on Free.
  ...
  return (
    <div className="min-h-screen bg-brand-bg font-sans text-brand-text antialiased">
      ...
      <main className="px-4 py-10 sm:px-6 sm:py-14">
          ...
          <WizardCard wide>
            ...
            {!requiresPlanSelection && (
              <div className="mt-6 flex items-start gap-3 rounded-xl border border-brand-border bg-white p-4 text-xs text-brand-text/80">
                <i className="fa-solid fa-circle-check mt-0.5 text-brand-green" />
                <span>
        ...
        {step === 4 && (
          <WizardCard wide>
            <h1 className="text-2xl font-black tracking-tight sm:text-3xl">Choose your plan</h1>
            <p className="mt-2 text-sm text-brand-muted">
              You&apos;re importing <strong className="text-brand-text">{totalRows} products</strong>. Pick the plan
              that fits.
            </p>
            ...
            <div className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-2">
              ...
              <button
                ...
                className={`relative rounded-2xl border-2 p-6 text-left transition-all ${
                  selectedPlan === 'free'
                    ? 'border-brand-accent bg-brand-accent-light/40 shadow-lg shadow-brand-accent/10'
                    : 'border-brand-border bg-white hover:border-brand-accent/40'
                }`}
              >
                {selectedPlan === 'free' && (
                  <span className="absolute -top-3 right-4 rounded-full bg-brand-accent px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-white">
                    Selected
                  </span>
                )}
                <p className="text-sm font-bold uppercase tracking-wide text-brand-muted">Free</p>
                <p className="mt-1 text-3xl font-black text-brand-text">
                  ₹0<span className="text-sm font-semibold text-brand-muted"> / forever</span>
                </p>
                <ul className="mt-5 space-y-2.5 text-sm">
                  ...
                  <li className="flex items-center gap-2.5 text-brand-muted/60">
                    <i className="fa-solid fa-xmark" /> Only first {FREE_PRODUCT_LIMIT} of your {totalRows} products
                    import
                  </li>
                </ul>
              </button>
              ...
              >
                <span className="absolute -top-3 left-4 rounded-full bg-brand-magenta px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-white">
                  Recommended
                </span>
                {selectedPlan === 'paid' && (
                  <span className="absolute -top-3 right-4 rounded-full bg-brand-accent px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-white">
                ...
                <p className="text-sm font-bold uppercase tracking-wide text-brand-muted">Pro</p>
                <p className="mt-1 text-3xl font-black text-brand-text">
                  ₹999<span className="text-sm font-semibold text-brand-muted"> / month</span>
                </p>
                <ul className="mt-5 space-y-2.5 text-sm">
                  <li className="flex items-center gap-2.5">
                    <i className="fa-solid fa-check text-brand-green" /> Unlimited products
                  </li>
                  <li className="flex items-center gap-2.5">
                    <i className="fa-solid fa-check text-brand-green" /> Unlimited catalogs
                  </li>
                  <li className="flex items-center gap-2.5">
                    <i className="fa-solid fa-check text-brand-green" /> Custom branding &amp; templates
                  </li>
                  <li className="flex items-center gap-2.5">
                    <i className="fa-solid fa-check text-brand-green" /> All {totalRows} products import
                  </li>
                </ul>
              </button>
            </div>
