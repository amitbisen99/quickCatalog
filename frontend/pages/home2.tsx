...
// COLOR EXPERIMENT — a copy of pages/index.tsx with the main brand color
// (brand-accent, #4f46e5) swapped for #C7FF01 everywhere it appears —
// buttons, badges, gradients, icon fills, table headers, the two
// radial-gradient panels, the CTA overlay. index.tsx itself is untouched.
// New tokens for this page only live in tailwind.config.js
// (home2-accent / home2-accent-light / home2-accent-dark / home2-accent-text)
// and styles/globals.css (.home2-text-gradient, .home2-hero-grid-bg,
// .home2-feature-tab-active). #C7FF01 is very light/high-luminance, so it
// only appears as a fill/border/shadow/gradient color — anywhere the old
// indigo was used as on-white *text* (links, icon badges, button labels)
// uses home2-accent-text (#4D6600, a dark olive shade of the same hue)
// instead, and anywhere text sat on a solid accent-colored fill it
// switched from white to dark (brand-text/home2-accent-text) — both
// needed so nothing goes unreadably pale-on-white or white-on-pale.
...
export default function Home({ indiaPrice, internationalPrice }: HomeProps) {
  ...
  return (
    <div className="bg-brand-bg font-sans text-brand-text antialiased overflow-x-hidden">
      <Seo
        title="Instant Catalog (Color Preview) — Build Stunning Product Catalogs in Minutes"
