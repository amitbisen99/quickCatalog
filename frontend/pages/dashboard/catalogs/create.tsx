...
function CreateCatalogWizard() {
  ...
  // The auto-map gate on Step 2 guarantees these are mapped when Preview
    ...
    <DashboardLayout title="Create Catalog">
      ...
      {step === 3 && parseResult && !creating && (
        <WizardCard wide>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Match each field to the right column from your file — we&apos;ll use this mapping to build your catalog.
          </p>

          <div className="mt-5 inline-block rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700">
            <strong className="font-bold text-gray-900">{parseResult.totalRows}</strong> products found in your file
          </div>
          ...
          <FooterNav
            ...
            nextDisabled={creating || !mappingValid}
          />
        </WizardCard>
      )}
