// CORS can't just check a static CLIENT_URL anymore — once a vendor has
...
// allowed too, for every catalog that vendor owns. Querying the DB on
// every single request would be wasteful, so this keeps an in-memory Set
// refreshed periodically instead — domain activation is already a
// multi-hour manual DNS process, so a brief staleness window here (at
// most REFRESH_INTERVAL_MS) is a non-issue.
  ...
  try {
    ...
    for (const vendor of vendors) {
      if (vendor.subdomainStatus === 'active' && vendor.subdomain && APP_BASE_DOMAIN) {
        next.add(`https://${vendor.subdomain}.${APP_BASE_DOMAIN}`);
      }
      if (vendor.customDomainStatus === 'active' && vendor.customDomain) {
        next.add(`https://${vendor.customDomain}`);
      }
    }
    cachedOrigins = next;
  } catch (err) {
